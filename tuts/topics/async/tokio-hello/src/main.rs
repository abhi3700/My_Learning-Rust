...
//!    The different scopes in this code are used to ensure that the lock on the `Mutex` is released
//!    as soon as it's no longer needed. In Rust, a `MutexGuard` (the type returned by
//!    `lock().await`) is automatically unlocked when it goes out of scope. By using separate scopes
//!    for each operation, the code ensures that the lock is released immediately after each
//!    operation completes, allowing other tasks to acquire the lock if needed. If all operations
//!    were performed in the same scope, the lock would be held for the entire duration of the
//!    scope, potentially preventing other tasks from acquiring the lock even when it's not actively
//!    being used. This could lead to unnecessary contention and reduced concurrency. So, the use of
//!    different scopes in this code is a way to manage the lifetime of the `MutexGuard` and ensure
//!    that locks are held for the shortest time possible. Use `RwLock` instead of Mutex. But not
//!    recommended in this as it won't add any efficiency. If multiple threads are frequently trying
//!    to acquire the same lock, it can lead to performance issues. Consider using finer-grained
//!    locks, or other synchronization primitives like RwLock which allows multiple readers or one
//!    writer at a time.
