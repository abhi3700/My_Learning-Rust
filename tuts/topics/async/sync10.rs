...
//! - The former could use the same counter local variable in the main thread. But, the latter required to
//!     clone the local variable `counter`. Hence, Arc smart pointer is used for using its clones in multiple threads.
//!
//! In short, M-1 can't be considered as the solution for the given problem as the Q. mentioned async task(s).
...
pub(crate) async fn main() {
    ...
    // M-2.2
    // Run concurrently as they are put together. This means that `task_1` or `task_2` might start first.
