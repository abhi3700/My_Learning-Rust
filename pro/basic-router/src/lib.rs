...
fn get(path: &str) -> Request<'_> {
	Request { method: METHOD::GET, path, header: "content/type: JSON", body: "" }
}
...
impl BasicApiRouter {
	fn new(api_url: &'static str) -> Self {
		Self { url: api_url, routes: HashMap::new() }
	}
...
fn api_status() -> Response {
	Response { status_code: 200, message: "API running successfully", body: "".to_string() }
}

fn not_found() -> Response {
	Response { status_code: 404, message: "Not found", body: "".to_string() }
}
